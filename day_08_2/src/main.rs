...
fn main() {
    ...
    let mut prog: Vec<(&str, i32)> = input
        .lines()
        .map(|line| {
    ...
    // flip each nop or jmp one at a time from the end
    for changed_instruction in 0..prog.len() {
        println!("Change this -> {}", changed_instruction);
            ...
            && instruction_pointer < prog.len()
        {
            ...
            // );
            if instruction_pointer != changed_instruction {
                match ins {
                ...
                match ins {
                    "acc" => acc(&mut instruction_pointer, &mut accumulator, var),
                    "nop" => jmp(&mut instruction_pointer, var),
                    "jmp" => nop(&mut instruction_pointer),
                    _ => panic!(),
                }
            }
        }
