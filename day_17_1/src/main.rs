...
fn main() {
    ...
    for i in 0..input_vec.len() {
        for j in 0..input_vec[i].len() {
            ...
            if input_vec[i][j] {
                insert_new_active_cube((0, i as i32, j as i32), &mut infinicube);
            } else {
                infinicube.insert((0, i as i32, j as i32), false);
            }
        }
    }
...
fn adjacent_to(origin: Cube) -> Vec<Cube> {
    ...
    for x in (origin.0) - 1..(origin.0) + 2 {
        for y in (origin.1) - 1..(origin.1) + 2 {
            for z in (origin.2) - 1..(origin.2) + 2 {
                adjacent_points.push((x, y, z));
            }
        }
    }
    adjacent_points.remove(13); // Remove origin from list of adjacent points
    adjacent_points
}
